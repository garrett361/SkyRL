"""Integration tests for the OpenEnv verifier with memlimited_coding_env.

Same test cases as test_openenv_verifier_integration.py but spins up a
MemlimitedPythonCodeActEnv (subprocess-isolated) server instead of the
in-process PythonCodeActEnv.  This validates that the verifier works
end-to-end with the memory-limited executor that TORL uses in production.
...
class _VerifierCompatibleMemlimitedEnv(MemlimitedPythonCodeActEnv):
    """MemlimitedPythonCodeActEnv with extra authorized imports for fn_name wrapper.
    ...
    so the standard MemlimitedPythonCodeActEnv works unmodified.
    """

    def __init__(self):
        super().__init__(additional_imports=_WRAPPER_IMPORTS)
...
def openenv_server():
    """Start a memlimited OpenEnv HTTP server on a random port for the module."""
    port = _free_port()
    app = create_fastapi_app(
        _VerifierCompatibleMemlimitedEnv, CodeAction, CodeObservation
    )
...
class TestCallOpenenvApiIntegration:
    def test_simple_print(self, openenv_server):
        resp, err = call_openenv_api(
            openenv_server, "print('hello')", timeout_s=TEST_TIMEOUT
        )
    ...
    def test_arithmetic(self, openenv_server):
        resp, err = call_openenv_api(
            openenv_server, "print(2 + 3)", timeout_s=TEST_TIMEOUT
        )
    ...
    def test_runtime_error_returns_nonzero_exit(self, openenv_server):
        resp, err = call_openenv_api(
            openenv_server, "1 / 0", timeout_s=TEST_TIMEOUT
        )
